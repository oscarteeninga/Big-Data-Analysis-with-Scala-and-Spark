...
object TimeUsage extends TimeUsageInterface {
  ...
  /** @return The initial data frame columns partitioned in three groups: primary needs (sleeping, eating, etc.),
    ...
    */
  val needPrefix = List("t01", "t03", "t11", "t1801", "t1803")
  val workPrefix = List("t05", "t1805")
  val otherPrefix = List("t02", "t04", "t06", "t07", "t08", "t09", "t10", "t12", "t13", "t14", "t15", "t16", "t18")

  def classifiedColumns(columnNames: List[String]): (List[Column], List[Column], List[Column]) = {
    if (columnNames.isEmpty) (Nil, Nil, Nil)
    else {
      val (needs, works, others) = classifiedColumns(columnNames.tail)
      columnNames.head match {
        case need if needPrefix.exists(need.startsWith) => (new Column(need) :: needs, works, others)
        case work if workPrefix.exists(work.startsWith) => (needs, new Column(work) :: works, others)
        case other if otherPrefix.exists(other.startsWith) => (needs, works, new Column(other) :: others)
        case _ => (needs, works, others)
      }
    }
  }
  ...
  def timeUsageSummary(
    ...
    val workingStatusProjection: Column = when('telfs >= 1.0 && 'telfs < 3, "working")
    ...
    def sum(columns: List[Column]): Column = {
      columns.reduce(_ + _).cast(DoubleType).divide(60)
    }
    val primaryNeedsProjection: Column = sum(primaryNeedsColumns).as("primaryNeeds")
    val workProjection: Column = sum(workColumns).as("work")
    val otherProjection: Column = sum(otherColumns).as("other")
    df
  ...
  def timeUsageGroupedTyped(summed: Dataset[TimeUsageRow]): Dataset[TimeUsageRow] = {
    ...
    summed
      .groupByKey(key => (key.working, key.sex, key.age))
      .agg(
